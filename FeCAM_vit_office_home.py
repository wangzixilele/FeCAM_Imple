...
def get_dataset(id=0):
    images = []
    labels = []
    paths = os.listdir("/public/home/imgbreaker/CIUDA/ProCA-main/ProCA-main/dataset/office-home")
    for i in range(id*10,(id+1)*10):
        if not os.path.isdir(os.path.join("/public/home/imgbreaker/CIUDA/ProCA-main/ProCA-main/dataset/office-home",paths[i])):
            continue
        for j in os.listdir(os.path.join("/public/home/imgbreaker/CIUDA/ProCA-main/ProCA-main/dataset/office-home",paths[i])):
            images.append(Image.open(os.path.join("/public/home/imgbreaker/CIUDA/ProCA-main/ProCA-main/dataset/office-home",paths[i],j)).convert("RGB"))
            labels.append(i)
    return officeHome(images,labels,transform=transforms.Compose([transforms.Resize(256),transforms.CenterCrop(224),transforms.ToTensor()]))
...
for task in range(6):
    print(f"Task {task}")
    office_home = get_dataset(task)
    size_of_dataset = len(office_home)
    train_dataset , test_dataset_ = torch.utils.data.random_split(office_home, [int(0.8 * size_of_dataset), size_of_dataset - int(0.8 * size_of_dataset)])
